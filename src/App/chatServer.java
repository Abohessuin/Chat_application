...
public void setSocket(Socket socket) throws IOException {
	...
	//	System.out.println("num"+H);
		...
		case 4: {
			...
			boolean Foundname =this.SD.isAccountFoundUserName(CSP.getUsername());
			boolean Foundpass =this.SD.isAccountFoundpass(CSP.getPass());
			if (Found) {
		...
		case 5: {
			...
			String Myname =this.SD.getId(this.clientSocket);
			Account A=this.SD.getAccount(Myname);
				//A.printacc();
		...
		case 8: {
			...
			if(checkwordsBanned(serverDatabaseUsingODS.getBannedwords(), pmsg)) {
				...
				System.out.println(user.getStatues());
			...
			else if(user.getStatues().equals("banned")) {
				respondeToClient("you got banned due to unbehaviors words in msg", this.clientSocket);
			}else {
		...
		case 3: {
			...
			if(this.SD.isAccountFounduser(CSP.getCommunicateWith().get(0))) {
				...
				System.out.println(Myname + ": contacts");
				System.out.println(A.getContacts() +"end");
				this.SD.printclients();
